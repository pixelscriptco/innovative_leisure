$(function () {
    $('.project-li').find('a').addClass('active');
    callBackDataTables();

    $('#new').on('click', function () {
        AppModal.loadData('admin/project/create', 'New Project', projectCall, {size: 'large'});
    });
});
    ...
    $('#dataTable').DataTable({
        ...
        fnDrawCallback: function (settings) {
            $('.menu-edit').on('click', function () {
                ...
                AppModal.loadData(`admin/project/${id}/edit`, `Role - ${name}`, projectCall, {size: 'large'});
            });

            $('.menu-delete').on('click', function () {
                let id = $(this).closest('tr').data('id');

                Ajax.delete(`/admin/project/${id}/delete`);
            });
        },
        buttons: DT.exportButtons('Projects')
    });
}
